...
def eval(epoch, config, model, validating_data, logger, visualizer=None, tensorboardTag='Tag'):
    model.eval()
    total_loss = 0
    total_dist = 0
    total_word = 0
    ...
    with torch.no_grad():
        for step, (inputs, inputs_length, targets, targets_length) in enumerate(validating_data):
            ...
            if step % config.training.show_interval == 0:
                process = step / batch_steps * 100
                logger.info('-%s-Epoch:%d(%.5f%%), CER: %.5f %%' % (tensorboardTag,epoch, process, cer))

    val_loss = total_loss / (step + 1)
    logger.info('-%s-Epoch:%4d, AverageLoss:%.5f, AverageCER: %.5f %%' %
                (tensorboardTag,epoch, val_loss, cer))

    if visualizer is not None:
        visualizer.add_scalar('cer'+'/'+tensorboardTag, cer, epoch)
...
def main():
    ...
    config2 = AttrDict(config.copy())
    config2.data.base_path = './AIhub'
    ...
    if config.data.random_split == True:
        ...
        test_dataset = AudioDataset(config2, 'test')
   ...
   # summary(model, (1, 1375, 80))
    ...
    for epoch in range(start_epoch, config.training.epochs):
        ...
        _ = eval(epoch, config, model, validate_data, logger, visualizer, 'eng_keyword')
        _ = eval(epoch, config, model, test_data, logger, visualizer, 'aihub')
