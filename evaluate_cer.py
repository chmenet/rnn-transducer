...
def main():
   ...
   # summary(model, (1, 1375, 80))
    ...
    val_cer = eval(start_epoch, config, model, validate_data, logger, visualizer)
    test_cer = eval(start_epoch, config, model, test_data, logger, visualizer)

    #logger.info('---training CER: {:0.5f}'.format(train_cer))
    logger.info('---validation CER: {:0.5f}'.format(val_cer))
    logger.info('---test CER: {:0.5f}'.format(test_cer))
