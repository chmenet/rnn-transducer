...
import rnnt.layers_espnet as layers
...
def deploy(config_path, output):
    ...
    config.hparam.n_mel_channels = 80
    config.hparam.sampling_rate = 16000
    config.hparam.filter_length = 512
    config.hparam.hop_length = 160
    config.hparam.win_length = 512
    config.hparam.mel_fmin = 0.0
    config.hparam.mel_fmax = 8000.0
    config.hparam.max_wav_value = 32768.0
    config.hparam.max_abs_mel_value = 4.0
...
if __name__ == '__main__':
    """
        usage
        python deployNMelExtractor_espnet.py
    """
    parser = argparse.ArgumentParser()
    parser.add_argument('-config', type=str, default='deploy.yaml')
    parser.add_argument('-output', type=str, default='nSTFT_espnet.pt')
